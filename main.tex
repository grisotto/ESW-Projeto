...
Em desenvolvimento de softwares, etapas de teste do software são essenciais para garantir um produto final de qualidade, sendo que a realização de testes busca minimizar o número de erros no software. No entanto, nem sempre é possível eliminar todos os erros de um software por meio de testes, sendo que a minimização dos erros é um fator dependente dos casos de teste gerados. Dado esse cenário, a literatura tem investigado várias metodologias para geração de casos de teste para software. 

Publicações em "Teste de Software Baseado em Busca" tiveram início em 1976 com \cite{Miller:1976}. Foi depois de vários anos, em 1990, que Korel \cite{Korel:1990}, \cite{Korel:1992} deu continuidade a essa linha de pesquisa. Em testes de software baseados em busca, a geração de casos de testes por meio de algoritmos de busca, tal que as entradas são geradas aleatoriamente até que os objetivos do teste sejam satisfeito. Uma busca aleatória pode não ser muito eficiente para geração de casos de teste, quando o espaço de busca é muito maior do que o espaço de soluções viáveis \cite{McMinn:2011}. O ideal é utilizar algoritmos que podem ser guiados por determinados fatores da busca, como por exemplo, as meta-heurísticas, onde existe uma função para guiar a busca chamada de função de fitness (\emph{fitness function}). A função de fitness é dependente do problema sendo resolvido, e serve para guiar a busca por soluções, retornando valores que indicam a qualidade de cada solução encontrada. Pode-se observar então que a escolha de uma função de fitness eficiente é essencial para o bom desempenho da meta-heurística, e portanto, um bom desempenho da busca.

A seguir, são apresentados alguns exemplos de aplicações de funções de fitness em testes de software baseados em busca, presentes em \cite{McMinn:2011}:

\begin{itemize}
\item Testes temporais: busca aproximar os tempos de execução para o melhor e pior caso para componentes de um sistema. Muitas vezes, esses podem ser aproximados com análise estática. No entanto, a análise estática tende a ser conservativa, sobre-estimando o pior caso e sub-estimando o melhor caso. Uma forma de modelar esse problema como teste baseado em buscas, é definindo a função de fitness como sendo o tempo de execução do componente, para uma dada entrada. Nesse caso, o melhor caso será sobre-estimado e o pior caso sub-estimado. Portanto, pode-se melhorar a qualidade dos atributos encontrados pela busca, ao fazer análises juntamente com os atributos encontrados pela análise estática.
...
Uma abordagem comum na literatura é gerar um caso de teste para cada meta de cobertura individual (ex, \textit{branches in branch coverage}), e então os combinam em uma única suíte de teste. Porém, é difícil de predizer o tamanho resultante da suíte de testes, pois um caso de teste gerado para uma meta pode implicitamente também cobrir qualquer quantidade de metas de cobertura à frente. Ou seja, a ordem em que cada meta é escolhida pode desempenhar um papel importante, pois pode haver dependências entre elas. Há outros problemas quando se considera uma meta de cada vez, como metas quem podem ser mais difíceis de cobrir do que outras, ou até mesmo serem inviáveis \cite{fraser2013whole}.
...
uma vez que não se trata de heurística sobre (ou para) heurísticas, o que não é necessariamente verdade para todos os algoritmos.  

As próximas sub-seções descrevem as meta-heurísticas que tentaremos utilizar no problema de teste de software com o \textit{Evosuite}.

\subsection{Colonia de formigas}

Uma meta-heurística bio-inspirada que pode ser útil para o nosso projeto é Colônia de Formigas \cite{dorigo1999ant} ( do inglês \textit{Ant Colony Optimization} ) que surgiu da observação do comportamento das formigas reais e incluí técnicas de busca local. O algoritmo de colônia de formigas faz o uso de um algoritmo guloso para para construir a solução inicial e soluções viáveis. Outro ponto importante é a quantidade de feromônio depositado pelas formigas que dá a visibilidade que é a capacidade dada às formigas de observar os elementos que compõem a solução ou o caminho que elas estão percorrendo.
